...
export async function verifySignature(
  ...
  if (
    ...
    throw Error("Address is a contract");
  }
}
...
export const LOCALSTORAGE_KEY_DID = "DID";
...
export function setLocaleStorageDID(value: string): void {
  if (typeof window !== "undefined") {
    window.localStorage.setItem(LOCALSTORAGE_KEY_DID, `${value}`);
  }
}

export function getLocalStorageDID(): string | undefined {
  if (typeof window === "undefined") return undefined;
  const persisted = window.localStorage.getItem(LOCALSTORAGE_KEY_DID);
