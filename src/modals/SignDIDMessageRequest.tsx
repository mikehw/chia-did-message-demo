...
const SignDIDMessageRequest = (props: RequestModalProps) => {
  const { pending, result } = props;
  console.log('result', result);
  const resultData = result?.['result']?.toString() && JSON.parse(result['result']?.toString());
  return (
    <>
      {pending ? (
        <SModalContainer>
        ...
        <SModalContainer>
          ...
          <STable>
            {resultData && resultData?.data?.success ? <>
              <SRow>
                <SKey>Signed Message</SKey>
                <SValue>{JSON.stringify({did: props.currentDID, msg: props.currentMessage, sig: resultData.data.signature})}</SValue>
              </SRow></> : <>
...
export default SignDIDMessageRequest;
